...
/** Gmail / iCloud など、保存済みアカウント一覧に載せるメールか */
export function isSavedLoginProviderEmail(email: string): boolean {
  const key = providerKeyForEmail(email);
  return key === 'google' || key === 'apple';
}
