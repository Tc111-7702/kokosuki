      ...
      try {
        ...
        const spots: Suggestion[] = (spotData.suggestions ?? []).map(
          (s: { name: string; address: string }) => ({
            label: s.name, sublabel: s.address, type: 'spot' as const,
          })
        );

        const areas: Suggestion[] = (areaData.suggestions ?? []).map(
          (s: { label: string; sublabel?: string }) => ({
            label: s.label, sublabel: s.sublabel, type: 'area' as const,
          })
        );

        const stations: Suggestion[] = (stationData.suggestions ?? []).map(
          (s: { label: string; sublabel: string; lat: number; lng: number }) => ({
            label: s.label, sublabel: s.sublabel, type: 'station' as const,
            lat: s.lat, lng: s.lng,
          })
        );
        ...
        <div className="relative flex-1">
          ...
          >
            ...
            <input
              ...
              placeholder={isMobile ? '店舗・位置情報' : '店舗名・駅・都道府県/市区町村'}
                  ...
                  <div className="flex items-start gap-1.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        <span style={{ fontSize: 12, fontWeight: 600, color: '#222', lineHeight: 1.4 }} className="truncate">
                          {s.label}
                        </span>
                        {s.type === 'spot' && (
                          <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 8, flexShrink: 0, background: '#fef9c3', color: '#854d0e' }}>店舗</span>
                        )}
                        {s.type === 'station' && (
                          <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 8, flexShrink: 0, background: '#dbeafe', color: '#1d4ed8' }}>駅</span>
                        )}
                      </div>
              ...
              {contentSugg.map((s, i) => (
                <button key={i} className="w-full text-left px-3 py-2.5 active:bg-amber-50"
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#222', lineHeight: 1.4, flex: 1, textAlign: 'left' }}>
                      {s.label}
                    </span>
                    <span style={{
