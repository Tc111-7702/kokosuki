...
export function Location({ onAllow, onSkip, onBack }: Props) {
  const handleAllow = () => {
    navigator.geolocation.getCurrentPosition(() => onAllow(), () => onSkip());
  };
    ...
    <div className="flex flex-col min-h-screen bg-[#FFFEEF] px-6 pt-12 pb-8">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center">
        <div className="w-20 h-20 rounded-full flex items-center justify-center mb-6" style={{ background: '#FFF7E0' }}>
          <Navigation size={36} color="#F2B800" />
        </div>
        <h2 className="text-2xl font-black text-[#111] mb-3">近くのガチャを探そう</h2>
        <p className="text-[#888] text-[14px] mb-10 leading-relaxed">
          位置情報を使って、今いる場所の<br />近くにある在庫情報を表示します。
        </p>
